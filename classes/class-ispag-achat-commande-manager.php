 ...
 * Logging : Toutes les actions sont loguées dans ispag_achat_commande_manager.log.
 */
class ISPAG_Achat_Commande_Manager {
    ...
    //     );
    //     // $this->logger->log_user_action('achat_commande_manager', 'text_domain_loaded', [], null);
    // }
    ...
    public static function init() {
        ...
        // self::$instance->logger->log_user_action('achat_commande_manager', 'class_initialized', [], $user_id);
    }
    ...
    public function enqueue_ajax_script() {
        ...
        $user_id = get_current_user_id();
        // $this->logger->log_user_action('achat_commande_manager', 'ajax_script_enqueued', [], $user_id);
    }
    ...
    private function get_liste_projets() {
        ...
        $user_id = get_current_user_id();
        $this->logger->log_db_change('achat_commande_manager', $this->table_projets, 'SELECT', ['query' => $sql], $user_id);
    ...
    private function get_liste_fournisseurs() {
        ...
        $user_id = get_current_user_id();
        $this->logger->log_db_change('achat_commande_manager', $this->table_fournisseurs, 'SELECT', ['query' => $sql], $user_id);
    ...
    private function get_liste_etats_commande() {
        ...
        $user_id = get_current_user_id();
        $this->logger->log_db_change('achat_commande_manager', $this->table_etats, 'SELECT', ['query' => $sql], $user_id);
    ...
    private function enregistrer_nouvelle_commande($ref_commande_final, $id_fournisseur, $etat_commande, $abonne, $hubspot_deal_id) {
        ...
        if (empty($ref_commande_final) || empty($id_fournisseur) || empty($etat_commande)) {
            $this->logger->log('achat_commande_manager', 'ERROR: Champs obligatoires manquants pour enregistrer_nouvelle_commande', $user_id);
            return false;
        }

        $this->logger->log_db_change(
            'achat_commande_manager',
            $this->table_commandes,
            'INSERT_PREPARE',
            [
                'ref_commande' => $ref_commande_final,
        ...
        // Format: %s, %d, %d, %s, %d, %f, %d, %s, %d, %d, %s, %s
        $format = array(
        ...
        if ($result === false) {
            $this->logger->log('achat_commande_manager', 'ERROR: Échec de l\'insertion dans ' . $this->table_commandes . ' - ' . $this->wpdb->last_error, $user_id);
            return false;
        }
    ...
    public function display_achat_form_callback() {
        $user_id = get_current_user_id();
        $this->logger->log_user_action('achat_commande_manager', 'display_form_start', [], $user_id);
        ...
        $this->logger->log_user_action(
            ...
            'form_data_loaded',
            [
                'fournisseurs_count' => count($fournisseurs),
                'etats_count' => count($etats_commande),
                'projets_count' => count($projets)
            ],
            $user_id
        );
        ...
        <?php
        $this->logger->log_user_action('achat_commande_manager', 'display_form_complete', [], $user_id);
        return ob_get_clean();
    }
    ...
    public function handle_achat_ajax_submission() {
        $user_id = get_current_user_id();
        $this->logger->log_user_action('achat_commande_manager', 'ajax_submission_start', ['user_id' => $user_id], $user_id);

        // 1. Vérification de sécurité (Nonce)
        if (!check_ajax_referer('nouvelle_commande_action', 'nonce', false)) {
            $this->logger->log('achat_commande_manager', 'ERROR: Nonce invalide ou expiré', $user_id);
        ...
        if ($id_projet > 0) {
            ...
            if ($projet) {
                ...
                $this->logger->log_db_change(
                    ...
                    $this->table_projets,
                    'RESOLVE_REFERENCE',
                    ['id_projet' => $id_projet, 'resolved_ref' => $ref_commande_final],
                    $user_id
                );
            }
        }
        ...
        $this->logger->log_user_action(
            'achat_commande_manager',
            'user_subscriber_formatted',
            ['user_id' => $current_user->ID, 'abonne' => $abonne],
            $user_id
        );

        // 6. Validation essentielle
        if (empty($ref_commande_final) || $id_fournisseur <= 0 || $etat_commande <= 0) {
            $this->logger->log('achat_commande_manager', 'ERROR: Champs obligatoires manquants', $user_id);
        ...
        if ($nouvel_id) {
            ...
            $this->logger->log_user_action(
                'achat_commande_manager',
                'commande_saved_success',
                ['commande_id' => $nouvel_id, 'ref_commande' => $ref_commande_final],
                $user_id
            );
            $this->send_json_success($message, $nouvel_id);
        } else {
            $this->logger->log('achat_commande_manager', 'ERROR: Échec de l\'enregistrement de la commande', $user_id);
    ...
    private function send_json_success($message, $id) {
        $user_id = get_current_user_id();
        $this->logger->log_user_action('achat_commande_manager', 'json_success_sent', ['message' => $message, 'id' => $id], $user_id);
    ...
    private function send_json_error($message, $code) {
        $user_id = get_current_user_id();
        $this->logger->log('achat_commande_manager', 'ERROR: JSON error sent - ' . $message, $user_id);
